...
def make_binary_event_quest(quest_data):
    ...
    # size: 44
    # offset: 0x0000
    data += pad(quest_info['name'], 40)  # Size 0x28
    ...
    # size: 92
    # offset: 0x002E
    data += pad(quest_info['description'], 80)
    ...
    # offset: 0x008C
    data += pad(
        objective_details['subquest_1']['description'],
        0x29
    )
    ...
    # offset: 0x00B5
    data += pad(
        objective_details['subquest_2']['description'],
        0x29
    )
    ...
    # size: 92
    # offset: 0x00DE
    data += pad(quest_info['success_message'], 0x5C)
    ...
    # size: 256
    # offset: 0x01C9
    data += pad(quest_info['details'], 0x100)  # b'\0' * 0x100
...
def read_quest_sm_data(fname):
    ...
    # JSON data (including fname) was processed by byteify
    data_path = path.join(b"event", fname)
...
def byteify(obj):
    # type: (Any) -> dict[Any, Any]|list[Any]|bytes
    if isinstance(obj, dict):
        return {key: byteify(value)
