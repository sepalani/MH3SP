...
class State(object):
    ...
    def get_servers(self, include_ids=False, code=None):
        if not self.cache:
            return []
        server_ids, servers = self.cache.get_server_list(include_ids=True, code=code)
        below, above = [], []
        below_ids, above_ids = [], []
    ...
    def update_capcom_id(self, session):
        ...
        server_session = self.capcom_ids.setdefault(session.capcom_id, {"name": "", "session": None})
