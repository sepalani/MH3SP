...
class CentralConnectionHandler(object):
    ...
    def RecvEventsData(self, server_id, data):
        self.cache.info("Got request for events data from server "+str(server_id)+"!")
        eq_binary = self.cache.get_updated_event_quest_binary(whitelist=server_id==WHITELIST_SERVER)
        self.send_packet(PacketTypes.EventsData, eq_binary)

    def RecvTradingPostData(self, server_id, data):
...
class RemoteConnectionHandler(object):
    ...
    def RecvServerIDList(self, data):
        ...
        self.cache.info("Sending request for events data!")
        try:
            self.send_packet(PacketTypes.EventsData, b"")
        except Exception as e:
            print(e)
            print("Failed to send request for events data!")
    ...
    def RecvEventsData(self, data):
        self.cache.debug("Got new event quest data!")
        try:
            success, message = self.cache.update_event_quest_from_binary(data)
            if success:
                self.cache.info("Updated new event quest data!")
            if len(message):
                self.cache.error(message)
        except Exception as e:
            import traceback
            print(e)
            print(traceback.format_exc())
        self.send_packet(PacketTypes.TradingPostData, b"")  # Uncomment when the central server is updated

    def RecvTradingPostData(self, data):
        self.cache.info("Got new trading post data!")
        try:
            success, message = self.cache.update_trading_post_from_binary(data)
            if success:
                self.cache.info("Updated new trading post data!")
            if len(message):
                self.cache.error(message)
        except Exception as e:
            import traceback
...
class Cache(Logger):
    def __init__(self, server_id, binary_loader, debug_mode=False,
                 log_to_file=False, log_to_console=False, log_to_window=False,
        ...
        if self.is_central_server:
            ...
            self.whitelist_binary_loader = QuestLoader(file_loc='event_whitelist/quest_rotation.json')
    ...
    def get_updated_event_quest_binary(self, whitelist=False):
        # type: None -> List[dict]
        if whitelist:
            return self.whitelist_binary_loader.dump_quest_to_bytes()
        else:
            return self.binary_loader.dump_quest_to_bytes()

    def update_event_quest_from_binary(self, eq_binary):
        return self.binary_loader.load_quests_from_bytes(eq_binary)

    def get_updated_trading_post_binary(self):
        # type: None -> List[List[List[List[int]]]]
        return self.binary_loader.dump_trading_post_to_bytes()

    def update_trading_post_from_binary(self, tp_binary):
        return self.binary_loader.load_trading_post_from_bytes(tp_binary)
    ...
    def get_server_list(self, include_ids=False, code=''):
        ...
        if include_ids:
            return list(servers.keys()), list(servers.values())
